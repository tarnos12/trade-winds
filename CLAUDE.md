...
**Phases 1–4 + Town Interiors + Phase 5 (groundwork + design-free content) + Economy v3 + Construction & building logistics (v0.11.0) + Gradual trade transfer (v0.12.0) + Two-part research & per-building upgrades (v0.13.0) + Research-tree overhaul & kingdom resource overview (v0.14.0: full-screen LTT-style tech tree with bands/pips/queue, per-building unlock nodes, autonomous castle buying, Burgher→Citizen display rename, top-left resource sidebar with price charts) DONE. v0.15.0: LTT panel parity + trade fleets (PP-A..E: level-scaled trader fleets, multi-good cargo, per-tier happiness/income, player-controlled castle market, city ledgers + budget charts, redesigned city/castle/house panels, map juice). IN FLIGHT: TV2 terrain v2 workflow (new tiles/deposits/mapgen presets); CC content chains fully spec'd (4-tier needs matrix incl. Aristocrats) launches after TV2. Standing directive: orchestrate work via dynamic Workflows (plan → implement → adversarial review), delegating to Sonnet/Opus subagents by complexity.**
