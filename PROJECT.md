...
**v0.49.0 — current on `claude/get-ready-suyvr6`.** Construction + build-cost + UI batch (lead + one UI subagent). TIMED CONSTRUCTION: `CONFIG.build` (baseSec peasant6/worker10/burgher14/aristocrat18, +2/upgrade, cap20); `Buildings.buildTime(b)` + `Buildings.constructionProgress(b)` → {frac,deliveredFrac,timeFrac}; Sim delivery step now advances `b._buildT` capped at deliveredFrac×buildTime and flips `built` only when materials AND time complete (progress = min(time, delivered)). T1 starter costs → wood (hut/lumberjack/potato_farm 10, sawmill 20; gold dropped) so they route through timed construction; `CONFIG.town.startStock` wood 20→40. renderer draws an on-map construction bar (time fill + faint delivered cap). Building DETAIL PANEL redesigned to reference (PanelUI subagent, town-ui.js + index.html `#buildingPanel` CSS): level banner badge + name + ✕; 3-button action row (★ priority `data-priority` / ⬆️ upgrade `data-upgrade` / 🗑 demolish `data-demolish`→`setMode("eraseBuilding")`); refined `renderProducerChain` (inputs chips → ⚙️ process + `Ns · P%` + live bar → output +batch + stock bar); construction bar via constructionProgress; worker/resident PORTRAIT avatars (keeps `data-slot` logic); priority-star footer; home body (needs rings/happiness) kept. City cards compact (carts-castle-ui.js): number on the avatar, Give/Take + cooldown hidden until SHIFT held (`revealBtns`). FOG shrunk (`startReveal` 6/8/10). Trade off-road DETOURS around mountains/water (pathing.js `offRoadRoute` BFS over road-passable terrain + `straightLine` fallback for map-less state/islands). Internal porter carry capped at 10. Tests re-baselined (starters wood/timed): board 825, sim 141, buildings 166, stats 34; suite green. Browser smoke: panel renders + opens, no errors. **Still TODO:** the CITY CENTER itself taking time to build (town-lifecycle change), and full porter logistics (need-based multi-type pickup + research upgrades). Prior — **v0.48.0.** UI pass (lead). HUD: brand removed; top-right `#topClock`/`#gameTimer` (game clock from `state.tick`/2, updated in mainloop fps block) + version badge moved there; top-left `#kingdomWrap` groups gold + `#btnResearch` (relocated, id kept so techtree-ui handler stays bound) + new `#btnCastleCenter` (input.js handler: ease camTarget to 0,0 + `CastleUI.openCastlePanel`); `#btnMute` relocated into hudMenu Game row (keeps audio.js binding). Building panel: NEW `renderProducerChain(town,b,def)` visual chain (inputs/source → process wheel + cycle time + live % via `Sim.buildingProgress` → output batch + stock bar) for built producers; text Output/Inputs kept as fallback. Selected-building map highlight: `window.TownUI.selectedBuilding` getter → `drawBuildings` draws a glow ring on the open building. Single-panel-at-a-time: `dismissOtherPanels()` (town-ui) + cross-module close calls so town/building/castle panels + scout selection are mutually exclusive (`closeBuildingPanel` now exported). Browser smoke (playwright) clean; suite 825/140/52/178. **Still TODO:** city-card compaction (image+number, shift→give/take), trader off-road mountain avoidance (pathing.js offRoadRoute still straight-line). Prior — **v0.47.0.** Map/economy/UX pass (lead + one UI subagent). Resource DENSITY: `CONFIG.map.depositDensity` (low 3/normal 6/high 10 clusters per ore type); `MapGen.generate` resolves `perType` from the resources tier (custom `tiers.resources` or preset `tiers.resources`, min 3) and spawns that many clusters of EACH of stone/clay/iron/coal/gold — fixes "1 gold node". Deposits SCATTER (extra tiles within hex-dist 2 of the seed, 60% adjacent/40% gapped) and rocky ores INTERMIX (`MIX=[stone,coal,iron]`, 28% per extra tile, band-respecting so tiers hold; clay/gold pure; seed always pure ⇒ ≥perType each). Mountains: seeds now drawn by rng from the upper-60% elevation with a min-separation (several semi-random ranges, not one central massif; `mtnSeeds` collected); `depositAffinity` retuned so iron/gold hug `mountains`, stone/coal prefer mountains-or-barren. NEW `Sim.buildingProgress(state,town,b)` (read-only) drives a per-building production PROGRESS BAR in `drawBuildings` (green producing/amber starved); internal porters now carry `Math.floor(have)` (the whole batch). Castle starterStock gains `potato:10`. Roads: `input.js` `roadRouteAB` BFS routes A→B around mountains/water (fallback to straight line). Scouts: `stepExplore` walks to the clicked tile even across discovered ground; `routeVisible` skips non-`road` terrain (mountains/water impassable); panel moved bottom-left; deselect via ESC/right-click/other-click (`input.js` calls `Scouts.deselect`). newGame sets `state.mode="pan"` (no pre-armed City tool). Always-on neutral hover highlight in pan mode (`drawHoverGhost`). NEW build-bar "⭐ Special" category (UISpecial subagent, town-ui.js only) holds Research Center + Advanced Provisioner. board.test re-baselined for density (825). **Still TODO (v0.48):** building detail-panel + full HUD-layout redesign to match the reference screenshot (city image-tokens w/ shift give-take, top-left research+castle icons next to gold, game timer, remove brand/sound text), selected-building map highlight, single-panel-at-a-time (scout/building/castle mutually exclusive), trader off-road mountain avoidance. Prior — **v0.46.0.** Advanced Provisioner building (lead): research-unlocked (`advanced_provisioner` logistics node) castle-adjacent placed building via a lightweight build-mode (`state.mode==="advProvisioner"`, one-shot). `Buildings.canPlaceAdvancedProvisioner`/`placeAdvancedProvisioner` (castle-adjacent, buildable, gold `CONFIG.advancedProvisioner.build.gold`, instant build) set `state.advancedProvisioner={q,r,built:true}` and switch on castle fish-buying (`castleTrade.fish`), which activates the Provisioner `advanced` line (1 fish + 1 potato → 2). Build-menu item shown only when researched && !built; renderer draws the 🍲 token + a castle-neighbour placement highlight; research.test re-baselined (logistics 8, kingdom 21, EXPECT 57). Prior — **v0.45.0** Scout units (ScoutAgent wrote src/scouts.js; lead scaffolding + integration). New impure IIFE module `src/scouts.js` (585 lines, MANIFEST + BUILD:scouts markers next to internal-traders) — real-time colored units (Red start; Green/Blue via research `scoutCount` = logistics nodes scouting_party T1 + ranger_lodge T2). Hooks: `Scouts.frame(dt)` in drawWithDpr (move/reveal/draw, gameSpeed-scaled), `Scouts.handleClick(q,r,e)` first-refusal on pan-clicks (select / set explore target), `Scouts.ensure` tops up to allowed count; `state.scouts` persisted. Behaviour: select via top-left `#scoutBar` icon or map click → bottom `#scoutPanel` (🎒 x/10, Explore/Return/Guard[disabled]) → Explore plants a colored flag, scout routes (roads 2×, visible-only fallback so it never stands on fog), pauses ~2s, lifts ring-1 fog 1 tile/provision around the flag; empties → returns to castle, refills 1/sec from state.provisions, resumes. `CONFIG.scouts` (cap 10, moveSpeed 3.4, etc.). scoutBar repositioned to top:200/left:12 (cleared the gold chip). Prior — **v0.44.0** Castle upgrades removed + provisions economy (lead): castle upgrade system stripped (progress.js Castle.canUpgrade/upgrade gone, carts-castle-ui upgrade box + handler removed, HUD/EventLog level readouts removed — castle is fixed-capacity; state.castleLevel stays 1 for save compat). NEW pure-core module `src/provisions.js` (`Provisioner`, MANIFEST + PURE_CORE markers): built-in provisioner converts castleStock potato → provisions on a per-line timer (`CONFIG.castle.provisions`: start 15, cap 30, basic 2 potato→1 every 6 ticks; advanced 1 fish+1 potato→2 reserved for the Advanced Provisioner building v0.45). save.js enables potato buying by default (`castleTrade.potato {enabled,limit:40}`) + persists provisions/_provTimers/advancedProvisioner; mainloop ticks Provisioner after CastleMarket; castle panel shows a Provisions section via castlePanelSections. Prior — **v0.43.0** MapGen terrain overhaul (MapAgent2, integrated by lead): patch paradigm — barren/desert/snow/water/mountains are filler, fertile & forest are discrete mixed-biome patches (`CONFIG.map.patchSizes` small/medium/big; fertile ≤~22% of land, was default ground); mountains are grown ranges with a map-wide reachability guarantee (`MapGen.ensureReachable` carves ≤8 obstacle hexes to reconnect walled-off roadable regions after `ensureCastleConnected`); deposit terrain-affinity (`CONFIG.map.depositAffinity`: clay→water, ore→barren/mountains) layered on the distance bands; snow at BOTH poles; NEW inland lakes (`CONFIG.map.lakes`/`lakeSize`) + winding rivers (`CONFIG.map.rivers`/`riverWidth`, `MapGen.carveRiver`, width drifts 1–5) that can host fish; fog start (`CONFIG.fog.startReveal` 10/15/20 by size, `generate()` returns `revealRadius`, save.js reveals it) with a guaranteed forest+fertile opening; two new Custom axes lakes/rivers (`CONFIG.mapTiers` + `applyTiers`; start panel auto-builds `#ssTier_lakes`/`#ssTier_rivers`). board.test 773→824. Determinism + 100% reachability + reveal-viability verified. Prior — **v0.42.0** City cap + one-shot founding (lead): `CONFIG.town.baseCityCap` (4) + additive research `cityCapBonus` via three administration-branch nodes (township_grants +3, provincial_rule +3, imperial_domain +2 → 4/7/10/12); `Buildings.cityCap(state)` enforced in `canPlaceTown`; the City build button shows count/cap and disables at cap; founding a city now returns to pan mode (was sticky → accidental repeats). New nodes carry materials in `RESEARCH_MATERIALS`; research.test re-baselined (kingdom 15→18, admin branch 8). Prior — **v0.41.0** Distance-tiered deposits + off-center castle (lead): the castle sits at axial (0,0) but is placed at a seeded OFF-CENTER board cell (`CONFIG.map.castleMargin`) — the grid loop offsets so (0,0) lands off-center, keeping all (0,0)-anchored code (spawn, connectivity, fog) intact while water falloff keys to the board centre and snow to the board top. Ore deposits now spawn in `[minFrac,maxFrac]` DISTANCE BANDS from the castle (`CONFIG.map.depositTiers` + `depositBands`, fraction of castle→farthest-hex): T1 stone [0,1], T2 clay/coal [0.33,1], T3 iron/gold [0.66,1]; a preset may override per-good via `deposits.<good>.band` (Highlands pulls coal/iron inward). Verified: castle offset varies by seed, all bands hold, determinism intact, full suite green. Prior — **v0.40.0** Economy granularity overhaul (EconAgent, integrated by lead): `town.stock` is now integer everywhere (fractional carry so nothing is lost); producing buildings bank output and release whole units on a per-building timer (`CONFIG.econ.productionIntervalSec` — extractor 8s / processor 12s, throughput unchanged); integer consumption via per-town carry while satisfaction still reads real fractional demand (satEMA smoothness preserved); trader/porter hauls are whole units ≥1; the 1-wood porter flicker is gone (batched production is the fix, no threshold hack); internal porters now scale movement with `state.gameSpeed` (MotionDev fade preserved). Prior — **v0.39.0** Custom Map + preset retune (MapAgent, integrated by lead): `CONFIG.mapTiers` (6 axes × 3 tiers) + `MapGen.applyTiers(base, sel)` build a resolved preset from a base + dropdown selection; start-screen gains a "Custom…" panel (`#ssCustom`, 6 `#ssTier_*` dropdowns, `#ssRegen`) with live no-save preview; custom `{preset,tiers}` persisted in the save. The 5 presets are retuned to read as distinct (Fertile/Oasis/Big World + new Highlands, Isles). Integration note: the agent worktrees forked from stale `main` (v0.34); lead re-removed reintroduced Kingdom-Events save lines and restored the v0.38.2 Main Menu wiring during merge. Prior — **v0.38.3** Castle land guarantee: `MapGen.ensureCastleCore` clears water/mountains from the castle's 7-hex core (deposits kept), and `MapGen.ensureCastleConnected` flood-fills road-passable land and carves a barren land-bridge if the castle is islanded — fixes the Oasis central-lake stranding. Prior — **v0.38.2** Added a **🏠 Main Menu** button to the ☰ Menu (`#btnMainMenu`, wired in start-screen.js): it `saveGame()`s, closes the popup, and reopens the title overlay via `StartScreen.show()` so Continue resumes the same game. Prior — **v0.38.1** Moved the bottom-center Build/Peasant bar down to the bottom edge (`#buildBar` bottom 96px→18px; the corner widgets it used to clear are gone). Prior — **v0.38.0** The board is now a **rectangle** (`CONFIG.map.rect`, 50×25 pointy-top hexes centered on the castle at axial (0,0)) instead of the old hex disc — MapGen builds the grid with a per-row axial shear so it reads as a true screen-space rectangle; water falloff switched to a Chebyshev (rectangular) edge distance, and the snow pole keys off the top row. Board test re-baselined (hex count = rect w·h = 1250; balance guard 50%→60% since the desert-dominant oasis preset is ~52% desert by design). Prior — **v0.37.0** System declutter: **retired the random Kingdom-Events system** (`events.js` / `Events` / `CONFIG.events` — Bumper Harvest / Demand Craze / Kingdom Fair) entirely, along with its event chip + kingdom banner; its guarded pure-core hooks (Sim farm/craze multipliers, Trade tariff waiver) and save fields are gone, and the module was dropped from the build MANIFEST (36→35). `showToast` (build-error feedback) and the bottom-right Event Log are kept. Also: the objectives panel no longer repeats the mission name below its header. Prior — **v0.36.0** UI declutter & feel pass (Let-Them-Trade style; agent team: lead HUD restructure + MotionDev camera/porter smoothing): corner-anchored HUD, a ☰ Menu popup housing all non-essential controls (seed/tools/reveal/help/debug), a collapsible bottom-right Event Log, right-edge objectives, and eased camera pan/zoom. Prior: feel/juice pass (agent team: EconDev/JuiceDev/AudioDev): the post-victory happiness **sawtooth is fixed** (satisfaction EMA, `CONFIG.needs.satSmoothing`; supplied estate holds a plateau), plus richer transaction/build VFX + chimney smoke and warmer audio cues + a market ambience bed. Brief: [`docs/JUICE_POLISH_BRIEF.md`](docs/JUICE_POLISH_BRIEF.md).
