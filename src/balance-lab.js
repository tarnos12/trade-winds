  ...
  const BalanceLab = (function () {
    ...
    function analyzeCity(city) {
      ...
      return {
        ...
        pop: pop, cap: cap, prod: prodMin, cons: consMin,
    ...
    function analyze(scenario) {
      ...
      for (const city of cities) {
        ...
        detail.push(c);
        for (const g in c.prod) { (perGood[g] = perGood[g] || { prod: 0, cons: 0 }).prod += c.prod[g]; }
        for (const g in c.cons) { (perGood[g] = perGood[g] || { prod: 0, cons: 0 }).cons += c.cons[g]; }
    ...
    function injectStyle() {
      ...
      styleEl.textContent = [
        ...
        "#balanceLabOverlay .bl-col{flex:1 1 0;min-width:0;overflow:auto;padding:14px}",
        "#balanceLabOverlay .bl-left{border-right:1px solid var(--panel-edge);max-width:46%}",
        ...
        "#balanceLabOverlay .bl-city{background:var(--panel);border:1px solid var(--panel-edge);border-radius:9px;padding:10px;margin-bottom:12px}",
        ...
        "#balanceLabOverlay .bl-brow{display:flex;align-items:center;gap:6px;margin:4px 0}",
        ...
        "#balanceLabOverlay .bl-brow select.ty{flex:1;min-width:0}",
        ...
        "#balanceLabOverlay .bl-pop{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;align-items:center}",
        "#balanceLabOverlay .bl-pop label{display:flex;flex-direction:column;font-size:10px;opacity:.7;gap:2px}",
    ...
    function renderComposer(host) {
      host.innerHTML = "";
      host.appendChild(el("h3", { text: "Composer — define your cities" }));
      host.appendChild(el("div", { class: "bl-note", html: "Add buildings (type × count × upgrade level) and set tier populations. Max pop is derived from placed houses; edit to override." }));

      const entries = buildingEntries();
      scn.cities.forEach((city, ci) => host.appendChild(renderCityCard(city, ci, entries)));
    ...
    function renderCityCard(city, ci, entries) {
      const cap = cityCapacity(expandBuildings(city));
      const card = el("div", { class: "bl-city" });
      const nm = el("input", { class: "nm", value: city.name || ("City #" + city.id) });
      nm.addEventListener("input", () => { city.name = nm.value; refreshAnalysis(); });
      const del = el("button", { class: "bl-x", text: "🗑", title: "remove city", onclick: () => { scn.cities.splice(ci, 1); renderAll(); } });
      card.appendChild(el("div", { class: "bl-city-hd" }, [nm, del]));

      (city.buildings || []).forEach((b, bi) => {
        const row = el("div", { class: "bl-brow" });
        const sel = el("select", { class: "ty" });
        for (const e of entries) sel.appendChild(el("option", { value: e.id, text: (e.def.name || e.id) + " (" + e.def.kind + ")", selected: e.id === b.typeId ? "selected" : null }));
        sel.value = b.typeId;
        sel.addEventListener("change", () => { b.typeId = sel.value; if (b.level > maxLevelFor(b.typeId)) b.level = maxLevelFor(b.typeId); renderAll(); });
        const cnt = el("input", { type: "number", min: "0", step: "1", value: String(b.count), title: "count" });
        cnt.addEventListener("input", () => { b.count = Math.max(0, Math.round(+cnt.value || 0)); refreshAnalysis(); });
        const lvlSel = el("select", { title: "upgrade level" });
        const mx = maxLevelFor(b.typeId);
        for (let L = 1; L <= mx; L++) lvlSel.appendChild(el("option", { value: String(L), text: "L" + L, selected: L === b.level ? "selected" : null }));
        lvlSel.value = String(b.level);
        lvlSel.disabled = mx === 1;
        lvlSel.addEventListener("change", () => { b.level = +lvlSel.value; refreshAnalysis(); });
        const rm = el("button", { class: "bl-x", text: "✕", onclick: () => { city.buildings.splice(bi, 1); renderAll(); } });
        row.appendChild(sel); row.appendChild(cnt); row.appendChild(el("span", { text: "×" })); row.appendChild(lvlSel); row.appendChild(rm);
        card.appendChild(row);
      });

      // add-building row
      const addSel = el("select", { class: "ty" });
      for (const e of entries) addSel.appendChild(el("option", { value: e.id, text: (e.def.name || e.id) + " (" + e.def.kind + ")" }));
      const addBtn = el("button", { class: "bl-btn", text: "＋ building", onclick: () => {
        city.buildings = city.buildings || [];
        city.buildings.push({ typeId: addSel.value, count: 1, level: 1 });
        renderAll();
      } });
      card.appendChild(el("div", { class: "bl-add" }, [addSel, addBtn]));

      // populations
      const popWrap = el("div", { class: "bl-pop" });
      popWrap.appendChild(el("span", { class: "bl-note", text: "Population (max from houses in parens):" }));
      for (const k of TIER_KEYS) {
        const cur = (city.pop && typeof city.pop[k] === "number") ? city.pop[k] : Math.round(cap[k] || 0);
        const inp = el("input", { type: "number", min: "0", step: "1", value: String(Math.round(cur)) });
        inp.addEventListener("input", () => {
          city.pop = city.pop || {};
          for (const t of TIER_KEYS) if (city.pop[t] == null) city.pop[t] = Math.round(cap[t] || 0);
          city.pop[k] = Math.max(0, Math.round(+inp.value || 0));
          refreshAnalysis();
        });
        popWrap.appendChild(el("label", {}, [k.slice(0, 4) + " (" + Math.round(cap[k] || 0) + ")", inp]));
      }
      const reset = el("button", { class: "bl-btn", text: "⟲ max", title: "reset to housing capacity", onclick: () => { city.pop = null; renderAll(); } });
      popWrap.appendChild(reset);
    ...
    function refreshAnalysis() {
      ...
      analysisHost.appendChild(el("h4", { text: "Per good (kingdom-wide, /min)" }));
      if (!goods.length) analysisHost.appendChild(el("div", { class: "bl-note", text: "No production or consumption yet — add buildings and population." }));
      for (const g of goods) {
        ...
        const numTxt = (surplus >= 0 ? "+" : "") + fmt(surplus);
        const num = el("div", { class: "bl-num " + (surplus >= -1e-6 ? "green" : "red"), text: numTxt });
        num.title = "prod " + fmt(pg.prod) + " − cons " + fmt(pg.cons);
        analysisHost.appendChild(el("div", { class: "bl-good" }, [el("div", { class: "lbl", text: goodName(g) }), bar, num]));
      }

      // per-city gold
      analysisHost.appendChild(el("h4", { text: "Per city — house income vs import cost (/min gold)" }));
      ...
      analysisHost.appendChild(tbl);
      analysisHost.appendChild(el("div", { class: "bl-note", html: "Income assumes needs are met (happiness 100 → ×1.6 tax). Import cost values each local deficit at its base price. Net &lt; 0 means tax can't fund the imports." }));
      ...
      analysisHost.appendChild(runWrap);
    }
    ...
    function open() {
      ...
      if (!overlay) {
        ...
        const bar = el("div", { id: "blBar" }, [el("span", { text: "🧪 Balance Lab — economy tester" })]);
        const resetBtn = el("button", { class: "bl-btn", text: "↺ Reset to example", onclick: () => { scn = defaultScenario(); renderAll(); } });
        ...
        const leftCol = el("div", { class: "bl-col bl-left" });
        analysisHost = el("div", { class: "bl-col bl-right" });
        body.appendChild(leftCol); body.appendChild(analysisHost);
