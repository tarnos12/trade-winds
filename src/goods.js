...
Object.assign(CONFIG, {
  ...
  buildings: {
    ...
    goldsmith: {
      ...
      // === BALPV2: gold_ring is UNIQUELY double-demanded — a direct aristocrat AND
      // burgher need AND the luxury_tailor's input (1 ring per luxury_clothes). At
      // ratePerWorker 1 one labor-scarce burgher goldsmith (~1.2 rings/tick) could not
      // cover the luxury_tailor draw (~2/tick) + the estate's direct need, so rings
      // starved (total ~3 vs ~80 for every other T3 good) and aristocrat happiness
      // decayed off its latch. 1→3 lets a SINGLE burgher goldsmith feed both sinks with
      // margin, holding the estate ≥99.5 — a throughput fix that relieves the burgher-
      // labor bottleneck WITHOUT re-tiering (goldsmith stays the one "prestige" good).
      // Inputs (gold/iron_tool) sit near their storage cap in the run, so the extra
      // yield converts to rings rather than stalling. ===
      terrain: null, workerTier: "burgher",
      inputs: { gold: 1, iron_tool: 1 }, output: { goodId: "gold_ring", ratePerWorker: 3 },
