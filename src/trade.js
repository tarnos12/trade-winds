...
(function () {
  ...
  Trade.tick = function (state) {
    ...
    for (const home of towns) {
      ...
      // v0.51 (H): the SAME trader also SELLS on this trip — fill spare capacity with
      // home's surplus goods that the DESTINATION city is short on, so a run can be
      // "buy 5 wood, sell 5 potato". Town sellers only (not the castle). The goods ship
      // out of home's stock now; they're paid for on arrival (minted tariff, §8), and any
      // the destination can't take/afford come straight back to home.
      const sellCargo = [];
      if (!sellerIsCastle && pick.seller) {
        ...
        for (const gid in CONFIG.goods) {
          ...
          const q = Math.floor(Math.min(sellCap, homeSurplus, sellerShort));
          if (q < 1) continue;
          home.stock[gid] = (home.stock[gid] || 0) - q;              // ship it out now
          sellCargo.push({ goodId: gid, qty: q, unitSell: priceOf(pick.seller, gid) });
          sellCap -= q;
        }
      }
      ...
      state.carts.push({
        ...
        sellCargo: sellCargo.length ? sellCargo : null,   // v0.51 (H): goods home sells TO the destination this trip
    ...
    for (const cart of state.carts) {
      ...
      if (cart.phase === "outbound") {
        ...
        if (!sellerIsCastle && !seller) {
          ...
          // v0.51 (H): the destination is gone, so its unsold sell-cargo comes home too.
          if (buyer && cart.sellCargo) { if (!buyer.stock) buyer.stock = {}; for (const it of cart.sellCargo) buyer.stock[it.goodId] = (buyer.stock[it.goodId] || 0) + (it.qty || 0); }
        ...
        // v0.51 (H): SELL home's outbound cargo to this destination on the same arrival.
        // Destination pays (up to its room + gold); home keeps the full sale and the
        // tariff is MINTED to the treasury (§8). Anything unsold returns to home now.
        if (cart.sellCargo && cart.sellCargo.length && seller) {
          for (const it of cart.sellCargo) {
            const capG2 = (CONFIG.town && CONFIG.town.storageCap);
            const room = capG2 ? Math.max(0, capG2 - (seller.stock[it.goodId] || 0)) : it.qty;
            const unit = it.unitSell || 0;
            let sell = Math.min(it.qty, room);
            if (unit > 0 && (seller.gold || 0) < unit * sell) sell = Math.floor((seller.gold || 0) / unit);
            if (sell < 0) sell = 0;
            const value = unit * sell;
            if (sell > 0) {
              if (!seller.stock) seller.stock = {};
            ...
            if (back > 0 && buyer) { if (!buyer.stock) buyer.stock = {}; buyer.stock[it.goodId] = (buyer.stock[it.goodId] || 0) + back; }
          }
          cart.sellCargo = null;
        }
